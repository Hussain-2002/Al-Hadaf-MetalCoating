...
export default function Electroplating() {
  ...
  const images = Array.from({ length: 12 }).map((_, i) => `/assets/gallery/electro-${i + 1}.jpg`);
  const videos = Array.from({ length: 6 }).map((_, i) => `/assets/videos/electro-${i + 1}.mp4`);

  // ✅ Unified form submit handler
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await submitComment(form, "Electroplating"); // 🎯 Service name sent here
      setSent(true);
    ...
    <div className="min-h-screen bg-[#ffffff] text-white overflow-x-hidden">
      ...
      <main className="pb-12 px-4 text-center relative">
        ...
        <p className="text-gray-800 mb-12 max-w-2xl mx-auto">
          <T>Precision. Protection. Perfection — the science of electroplating.</T>
        </p>
