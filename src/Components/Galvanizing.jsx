...
export default function Galvanizing() {
  ...
  // Mock gallery + video items (for demo)
  const images = Array.from({ length: 12 }).map(
    (_, i) => `/assets/gallery/galvanizing-${i + 1}.jpg`
  );
  const videos = Array.from({ length: 6 }).map(
    ...
    <div className="min-h-screen bg-[#ffffff] text-white overflow-x-hidden">
      ...
      <main className="pb-12 px-4 text-center relative">
        ...
        <p className="text-gray-800 mb-12 max-w-2xl mx-auto">
          <T>Shield your metals. Strengthen your future — the art of galvanizing.</T>
        </p>
            ...
            {["gallery", "video", "comment"].map((tab) => (
              <TooltipProvider key={tab}>
                <Tooltip>
                  ...
                  <TooltipContent className="bg-white text-black text-xs px-3 py-1 rounded-lg shadow-lg border border-gray-200">
                    <p>
                      {tab === "gallery"
                        ? "Work done by us"
                        : tab === "video"
